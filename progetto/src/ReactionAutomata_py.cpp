...
PYBIND11_MODULE(pyRA, m) {
  ...
  py::bind_vector<std::vector<DTYPE>>(m, "Vect");

  m.attr("ALPHABET") = ALPHABET;
  m.attr("RGENE_SIZE") = GeneSize<DTYPE,Reaction,ALPHABET>::size;
  py::class_<FRAGen, std::vector<DTYPE>>(m, "FGenome")
      .def(py::init<const std::vector<DTYPE> &>(), "genome"_a)
      .def("print", static_cast<void (FRAGen::*)(void) const>(&FRAGen::print))
      .def("__getitem__",static_cast<DTYPE & (FRAGen::*)(std::size_t)>(&FRAGen::operator[]));

  py::class_<ARAGen,std::vector<DTYPE>>(m, "AGenome")
      ...
      .def("__getitem__",static_cast<DTYPE & (ARAGen::*)(std::size_t)>(&ARAGen::operator[]));


//  py::bind_vector<ARAGen>(m, "AGenome");
  m.def("push",[](std::vector<DTYPE> & v){v.push_back(1);});
  ...
  m.def("fgenome_mutation", &fgenome_mutation);
  m.def("agenome_mutation", &agenome_mutation);

  m.def("fgenome_mutation_2Darr", [](py::array_t<DTYPE, py::array::c_style> & arr, const std::array<double,3>& probs, double mutation_prob, double m_prob, double rp_prob, double i_prob ){
        auto r = arr.mutable_unchecked<2>();
 //       auto r = x.mutable_unchecked<3>();
        auto buffer = arr.request();
        if (buffer.ndim == 2){
            fgenome_mutation_raw_vctr(r.mutable_data(0,0), r.shape(1), r.shape(0), (r.shape(1)/(ALPHABET) - 1)/3, mutation_prob, probs, m_prob, rp_prob, i_prob);
        }
  });
  m.def("agenome_mutation_2Darr", [](py::array_t<DTYPE, py::array::c_style> & arr, std::size_t n_reactions, const std::array<double,3>& probs, double mutation_prob, double m_prob, double rp_prob, double i_prob ){
        auto r = arr.mutable_unchecked<2>();
 //       auto r = x.mutable_unchecked<3>();
        auto buffer = arr.request();
        if (buffer.ndim == 2){
            agenome_mutation_raw_vctr(r.mutable_data(0,0), r.shape(1), r.shape(0), n_reactions, mutation_prob, probs, m_prob, rp_prob, i_prob);
        }
  });
/*  m.def("agenome_mutation_2Darr", [](py::array_t<DTYPE, py::array::c_style> & arr, std::size_t n_reactions, const std::array<double,3>& probs, double m_prob, double i_prob ){
      agenome_mutation_raw(static_cast<DTYPE *>(arr.mutable_data()), arr.size(), n_reactions, probs, m_prob, i_prob);
   });*/
