...
int main(){
    ...
    aut.verbose_react();
    aut.verbose_react();
    auto genome = genome_from_automaton(aut);
    genome.print();

    auto a_genome = genome_from_automaton(a_aut);
    a_genome.print();
    agenome_mutation(a_genome, {1.0,0.0,0.0}, 0.2,0.7,0.5);
    std::cout << "state mutated" << std::endl;
    agenome_mutation(a_genome, {0.0,1.0,0.0}, 0.2,0.7,0.5);
    std::cout << "reaction mutated" << std::endl;
    agenome_mutation(a_genome, {0.0,1.0,0.0}, 0.2,0.7,0.5);
    std::cout << "reaction mutated" << std::endl;
    agenome_mutation(a_genome, {0.0,0.0,1.0}, 0.2,0.7,0.5);
    std::cout << "final state mutated" << std::endl;
    a_genome.print();
    std::cout << "final state mutated" << std::endl;
    a_genome.print();


    //std::vector<std::vector<DTYPE>> inputs = {std::vector<DTYPE>{1},std::vector<DTYPE>{2}, std::vector<DTYPE>{3}};
    //std::cout << fitness_on_input_range<DTYPE, Reaction, ALPHABET, false>(aut1, [](std::vector<DTYPE> v){return 2*v[0];}, inputs, 100) << std::endl;
    //std::cout << fitness_on_input<DTYPE, Reaction, ALPHABET, false>(aut1, [](std::vector<DTYPE> v){return 2*v[0];}, inputs[1], 100);
    return 0;
}