...
void mutate_sequence_ow(N * gene_ptr, std::mt19937 random_gen, double m_prob, double rp_prob ,double i_prob){
...
template<typename N, std::size_t SIZE, bool CHEMICAL>
void mutate_sequence_ngh(N * gene_ptr, std::mt19937 random_gen, double variance, double rp_prob ,double i_prob){
    std::normal_distribution nrm_distribution(0.0,variance);
    std::discrete_distribution i_distribution{1.0-i_prob,i_prob}, rp_distribution{1.0-rp_prob,rp_prob};
    if constexpr(CHEMICAL){
        for(std::size_t i=0;i<SIZE;++i){
            gene_ptr[i] = static_cast<N>(std::max(nrm_distribution(random_gen) + gene_ptr[i], 0.0))*rp_distribution(random_gen);
            gene_ptr[i+SIZE] = static_cast<N>(std::max(nrm_distribution(random_gen) + gene_ptr[i+SIZE], 0.0))*rp_distribution(random_gen);
        }
    }else{
        for(std::size_t i=0;i<SIZE;++i){
            gene_ptr[i] = static_cast<N>(std::max(nrm_distribution(random_gen) + gene_ptr[i], 0.0))*rp_distribution(random_gen);
            gene_ptr[i+SIZE] = i_distribution(random_gen) && (gene_ptr[i]==0);
            gene_ptr[i+SIZE*2] = static_cast<N>(std::max(nrm_distribution(random_gen) + gene_ptr[i+SIZE*2], 0.0))*rp_distribution(random_gen);
        }
    }
    return;
}
template<typename N, std::size_t SIZE>
void mutate_short_sequence_ow(N * gene_ptr, std::mt19937 random_gen, double m_prob, double rp_prob){
    std::geometric_distribution ms_distribution(m_prob);
...
template<typename N, std::size_t SIZE>
void mutate_short_sequence_ngh(N * gene_ptr, std::mt19937 random_gen, double variance, double rp_prob){
    std::normal_distribution nrm_distribution(0.0, variance);
    std::discrete_distribution rp_distribution{1.0-rp_prob,rp_prob};
    for(std::size_t i=0;i<SIZE;++i){
        gene_ptr[i] = static_cast<N>(std::max(nrm_distribution(random_gen) + gene_ptr[i], 0.0))*rp_distribution(random_gen);
...
template<typename N, std::size_t SIZE>
void mutate_reaction_ow(CReaction<N,SIZE> & reaction, std::mt19937 random_gen, double m_prob, double rp_prob ,double i_prob){
    std::geometric_distribution ms_distribution(m_prob);
    std::discrete_distribution rp_distribution{1.0-rp_prob,rp_prob};
...
template<typename N, std::size_t SIZE>
void mutate_reaction_ngh(CReaction<N,SIZE> & reaction, std::mt19937 random_gen, double variance, double rp_prob ,double i_prob){
    std::normal_distribution nrm_distribution(variance);
    std::discrete_distribution rp_distribution{1.0-rp_prob,rp_prob};
    std::array<N,SIZE*2> reaction_gene;
    for(std::size_t i = 0; i < SIZE; ++i){
        reaction_gene[i] += static_cast<N>(nrm_distribution(random_gen)*rp_distribution(random_gen));
        reaction_gene[i+SIZE] += static_cast<N>(nrm_distribution(random_gen)*rp_distribution(random_gen));
    }
    reaction = reaction_from_gene(reaction_gene);
    return;
}
...
template<typename N, template<typename, std::size_t> class R, std::size_t SIZE, bool Acc, bool OW>
void mutation_raw_vctr(N * ptr, std::size_t len, std::size_t population, std::size_t n_reactions, double mutation_prob, const std::array<double,3>& probs, double variance, double rp_prob,double i_prob){
    ...
    for (int i = 0; i < population; i++){
        if(is_mutation(random_gen)){
            ...
            case 0:
                if constexpr(OW){
                    mutate_short_sequence_ow<N,SIZE>(&ptr[i*len], random_gen, variance, rp_prob);
                }else{
                    mutate_short_sequence_ngh<N,SIZE>(&ptr[i*len], random_gen, variance, rp_prob);
                }

            case 1:
                idx_r = reaction_idx(random_gen)*GeneSize<N,R,SIZE>::size;
                if constexpr(OW){
                    mutate_sequence_ow<N,SIZE,!(GeneSize<N,R,SIZE>::size-2*SIZE)>(&ptr[i*len + SIZE + idx_r ], random_gen, variance, rp_prob, i_prob);
                }else{
                    mutate_sequence_ngh<N,SIZE,!(GeneSize<N,R,SIZE>::size-2*SIZE)>(&ptr[i*len + SIZE + idx_r ], random_gen, variance, rp_prob, i_prob);
                }
            case 2:
                if constexpr(Acc){
                    std::size_t idx = _idx(random_gen);
                    if constexpr(OW){
                        mutate_short_sequence_ow<N,SIZE>(&ptr[i*len + len -idx*SIZE], random_gen ,variance, rp_prob);
                    }
                    mutate_short_sequence_ngh<N,SIZE>(&ptr[i*len + len -idx*SIZE], random_gen ,variance, rp_prob);
            }
        }
    }
    }
